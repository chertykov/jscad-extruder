...
var shaft;
var base;
var bearing = new Bearing(3, 10, 4); // 623
...
{
    return [
        { name: '_version', caption: 'Version', type: 'text', initial: "0.1 8-Mar-2015" },
        {
            name: 'debug',
...
function drive_gear ()
{
    ...
    return res.setColor (0.5, 0.5, 0.5, 0.3);
}
...
function Shaft(height)
{
    ...
    this.draw = function()
    {
        ...
        // hole
        var hole = cylinder ({r: Size.m3_r, h: this.h + 1, fn: 8});
        // m3 head hole
        var head_hole = (cylinder ({r: Size.m3_head_r, h: Size.m3_head_h + 1, fn: global.fn})
                     .translate ([0, 0, -1]));
...
function Lever ()
{
    ...
    this.draw = function ()
    {
        ...
        buttress = buttress.subtract (cylinder ({r: buttress_r + 1, h: this.h + 2, fn: 4})
                                      .translate ([-buttress_r / 2, buttress_r / 2, -1]));
        ...
        var ss_hole = union (cylinder ({r: Size.m3_screw_r, h: this.h * 3, center: true, fn: 8}),
                             cylinder ({r: Size.m3_head_r + this.clr, h: this.h, fn: global.fn})
                             .translate ([0, 0, this.h - 2])
                            ).translate ([-bearing_offset, 0, 0]);
        
        res = difference (res, ss_hole);

        res = res.union (cylinder ({r: Size.m3_head_r, h: 0.2})
                         .translate ([-bearing_offset, 0, (bh_h + this.h) / 2]));
        // Spring housing
...
function Base(hob_h)
{
    ...
    this.draw = function ()
    {
        ...
        // Mount holes
        var hole = cylinder ({r: Size.m3_r, h: this.shaft_base_h + 2, fn: global.fn});
        res = difference (res,
                          hole.translate ([nema.mount_dist, nema.mount_dist, -1]),
                          hole.translate ([-nema.mount_dist, nema.mount_dist, -1]));
        // Screw head housings.
        var head_house = (cylinder ({r: Size.m3_head_r, h: this.base_h, fn: global.fn})
                          .translate ([0, nema.mount_dist, this.base_h - Size.m3_head_h]));
